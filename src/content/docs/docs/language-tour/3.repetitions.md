...
```pomsky /^lala/ del=/lala(?=lali)/
lalalalalali
    ^
```

Now the regex engine has no more paths to explore, and gives up.
...
But it isn't finished yet, maybe there is a match somewhere else in the string! Since the regex engine already tried at the start of the string and failed, it skips to the next candidate, which is at the 3rd character:
...
The next step is matching the `'li'`#po part, which also succeeds, and the regex engine is
done. The matched substring is:

```pomsky ins=/lalalalali/
lalalalalali
```
